...
class SymbolTraverse extends DepthFirstAdapter
{
    ...
    public void caseASecondVarlist(ASecondVarlist node){
        ...
        if (curmeth.addParam(new Variable(lastid, assigntype)) == false) {
            System.out.println("couldn't add param " + lastid);
        }
    }
    ...
    public void caseAFirstIdlist(AFirstIdlist node){
        ...
        if (curmeth == null) {
            if (st.addVar(new Variable(lastid , assigntype)) == false) {
                System.out.println("couldn't add variable " + lastid);
            }
        } else {
...
public void caseAFirstMethodstmtseq(AFirstMethodstmtseq node){
    ...
    if (st.addMethod(curmeth) == false) {
        System.out.println("couldn't add method " + lastid);
    }
	node.getLparen().apply(this);
	node.getVarlist().apply(this);
	node.getRparen().apply(this);
